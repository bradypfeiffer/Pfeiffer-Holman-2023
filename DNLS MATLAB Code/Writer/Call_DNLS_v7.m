...
%% !!! Physics
    ...
    alpha = 1;      % plasma kinematics parameter
...
%% !!! Choose ProfileType = 1, 2, 7, 8, ..., 12 & Trial Objective    
    ...
    TrialObjective = ['Develop Landau Damping Numerical Method: test damped 1p bright soliton with Landau code with gamma = 0.05 and nu = 0'];
...
%% !!! Soliton Parameters: used in ProfileTypes 1, 2, 3, 11 and 12
    ...
    Soliton.lambda = 0.7;  % For a 1p soliton & actual eigenvalue is lambda_actual = lambda*bo
...
%% !!! Shock Parameters: used in ProfileTypes 7, 11 and 12
    Shock.Type  =  3    ;  % 1 --> 13 IS, 2 --> 23 IS on lower branch, 3 --> 23 IS on upper branch
    Shock.CR    = -0.8272 ;  % Compression ratio of 13 IS with -0.5 < IS.CR13 < 0 and CR23 = -1 - IS.CR13
                           % For Fast Shock 0 < CR < 1
    Shock.Iz    = -0.5; % Target Iz for IS
    ...
    Shock.x     = -0.2798*L;  % location of midpoint of Shock in range: -L --> +L
    Shock.xRamp = +0.2669*L;  % location of ramp ... in range: -L --> +L 
...
%% !!! Trial to Create, Overwrite and Archive Trial for ProfileType 12 
    ...
    TrialNumber = '4';  % This will create a folder, for example 'Trial 1' ... 
                        % for TrialNumber = 1 ... within folder
                        % 'Bradys_Trials' intended to store results from related trial runs
    RunNumber_start = 21;% the run number to be created with the 'Trial_N' folder
    RunNumber_end   = RunNumber_start; % sets values for start/end limits on for-loop surrounding call to Numerical Method
                        ...
                        % variables were structured to allow the code to
                        % automatically loop through a sequence of trials
...
%     end
    ...
    TrialID   = strcat(TrialNumber,'_',int2str(RunNumber_start)); % "#-#' format
    ...
    TrialValues = {Ttitle ,  TrialNumber , int2str(RunNumber_start),  TrialName,  TimeStamp , TrialObjective , TrialID};
    Trial = containers.Map(TrialKeys, TrialValues);
    
    FolderValues = {Ftitle,  ActiveArchiveBase ,  TrialFolderAbs, BfieldFolderAbs, B_field_Tr_Rn_txt, B_field_Tr_mat};
...
%   ... if a 1p soliton is to be spliced into either hybrid type profile
    ...
    onePsign = Soliton.sign;    % sign = +1 ... 'bright soliton'; sign = -1 ... for a 'dark soliton'
    prfmsg = sprintf('Defining 1psoliton profile with %s = %4.4f and sign = %4.0f',lambdaChar,lambda, onePsign);
    prfmsg
    ...
    PrflValues = { Prfltitle,   Profile ,  v ,  lambda ,   onePsign ,  bo  ,  0.1*L  };
...
if (ProfileType == 10)
    ...
    RunNumber_end   = RunNumber_start; % Sorry, multiple trial features have not been implemented.
    RunNumberNew    = num2str(RunNumber_start);
...
Profile = 'Hybrid Shock-Soliton';
    ...
    %      finalized soliton type and sign (if 1p) & 'eigenfactors':
...
if Soliton.Type == 1
    ...
    PrflOnePSolitonValues = { Prfltitle,  Profile1 ,  vsol         ,  lambda ,   onePsign ,  b_soliton  ,  1/(lambda*b_soliton^2)  };
...
%     debug1pgraph = 1;
    ...
    %*********************************************************
    prfmsg = sprintf('Finalized 1psoliton profile with %s = (%4.4f)bo with bo = %4.4f and sign = %4.0f',lambdaChar,lambda,b_soliton, onePsign);
    prfmsg
...
NTestSoliton = 10; Tol_Soliton = 0.001; b_ref = 1; % Potentially useful in later development to allow user to adjust tolerance in selecting boundaries
...
% Construct Parameters for New Bfield Trial and Run #'s
        ...
        RunNumberNew   = RunNumber_start;   % Run   # of B-field being created
...
%     [PStr, b1, MSG, flag] = Read_Bfield_Line(ArchiveBase, TrialNumber1,RunNumber1, LineNumber1);
   ...
   %% Define Prfl2 for Soliton and join with PrflHybrdSplice
    ...
    PrflValues2 = { Prfltitle,  Profile2 ,    v   ,  lambda ,   onePsign ,  b1p };
...
if (BFieldarchive == 1) && Proceed == 1
    ...
    Prflkeys = Prfl.keys;
    Prflvals = Prfl.values;
    DNLS_Bfield_Header_v2(PStr,'w', RunNumber_start, t);
    Prfl = PStr.PrflCon;
    Prflkeys = Prfl.keys;
    Prflvals = Prfl.values;
    elapsed = num2str(cputime - t); t = cputime;
