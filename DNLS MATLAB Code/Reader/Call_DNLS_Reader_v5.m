...
%% *************************************************************************
    ...
    TrialNumber = '8';% The trial number stored in your Archive to analyze
    RunNumber = '2';  % ... well, the name pretty much says it all ...
    LineNumberStart = 40;   % B-field line number to study
    LineNumberLast = 40; 
    % The loop below is helpful for evaluating real eigenvalues
    % Set EVZoom = 0 and boolREV = 1 and turn off graphing and videos 
    % and complex eigenvalue searches
    % When I re-ran the short Total time until coalescence, the L was set
    % to L = 15, N = 2^11 which seemed to be adequate
    for LineNumber = LineNumberStart:LineNumberLast 

        
        %% CREATE B-FIELD GRAPH, HODOGRAM OR VIDEO?   
        boolBgraph = 0; % '1' to graph to Display B-field
        ...
        %% OPTIONS FOR EIGENVALUE SPECTRUM ANALYSIS   
            EVZoom = 0; %  = '+1' if user is to chose a smaller section of B-field for Real EV evaluation
                        %This can improve accuracy and speed up calculations
            boolEnergy = 1; % Evaluate total magnetic energy stored in profile
        ...
        zinit = 0.3+1i*0.79; %initial guess for Newton's Method if boolRoot = 1
        ...
        %the corresponding results are reported
                ...
                LComplexEval = [0.3, 0.43; 0.72, .82]; %range of real values to study ... [Real Min, Real Max; Im Min, Im Max]
    ...
    %% CREATES BoolCon and EvalCon to pass User Parameters to DNLS_Reader
        boolKeys    = {'title'       , 'boolBgraph', 'boolHodogram', 'boolVideo','boolHodogramVideo', 'EVZoom', 'boolEnergy', 'boolREV', 'boolRoot','zinit', 'boolCEV','boolCEVgraph', 'boolRAD', 'boolRADgraph','boolIz', 'boolSpeedCheck', 'boolDevelopment' , 'NMonitors', 'boolPeakOscAnalysis'};
        boolValues  = {'User Choices',  boolBgraph ,  boolHodogram , boolVideo  , boolHodogramVideo ,  EVZoom ,  boolEnergy ,  boolREV ,  boolRoot , zinit ,  boolCEV , boolCEVgraph ,  boolRAD ,  boolRADgraph , boolIz ,  boolSpeedCheck , boolDevelopment   ,  NMonitors ,  boolPeakOscAnalysis};
    ...
    %% Call to DNLS_Reader
        if Proceed == 1
            ...
            DNLS_Reader_Handle(BoolCon, EvalCon, ActiveArchiveBase, TrialNumber, RunNumber, LineNumber)    
        end
